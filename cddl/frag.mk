CDDLC_FRAGS := cbor-time-tag.cddl

EPOCH_MARKER_FRAGS := epoch-marker.cddl
EPOCH_MARKER_FRAGS += cbor-time-tag.cddl
EPOCH_MARKER_FRAGS += classical-rfc3161-tst-info.cddl
EPOCH_MARKER_FRAGS += multi-nonce-list.cddl
EPOCH_MARKER_FRAGS += multi-nonce.cddl
EPOCH_MARKER_FRAGS += strictly-monotonic-counter.cddl
EPOCH_MARKER_FRAGS += tst-info.cddl
EPOCH_MARKER_FRAGS += non-empty.cddl

EPOCH_MARKER_EXAMPLES := examples/1.diag

CWT_EM_FRAGS := cwt-em.cddl
CWT_EM_FRAGS += cose.cddl
CWT_EM_FRAGS += epoch-marker.cddl
CWT_EM_FRAGS += cbor-time-tag.cddl
CWT_EM_FRAGS += classical-rfc3161-tst-info.cddl
CWT_EM_FRAGS += multi-nonce-list.cddl
CWT_EM_FRAGS += multi-nonce.cddl
CWT_EM_FRAGS += strictly-monotonic-counter.cddl
CWT_EM_FRAGS += tst-info.cddl
CWT_EM_FRAGS += non-empty.cddl

CWT_EM_EXAMPLES := examples/1-cwt.diag
