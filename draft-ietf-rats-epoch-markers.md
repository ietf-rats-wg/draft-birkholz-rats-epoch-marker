...
informative:
  ...
  TCG-TPM2:
    ...
    target: https://trustedcomputinggroup.org/wp-content/uploads/Trusted-Platform-Module-2.0-Library-Part-2-Version-184_pub.pdf
    date: 2025-03
    rc: Version 184
...
The default container for Epoch Bell-signed Epoch Markers (see {{sec-signature-reqs}}) described in this document is CBOR Web Tokens (CWT) {{-CWT}}.
...
As some Epoch Marker types represent certain timestamp variants, these Epoch Markers or the secure conveyance method they are used in do not necessarily have a hard-coded message imprint, as is always the case with {{-TSA}} TimeStampTokens.
This means that not all Epoch Marker types support binding a message to an Epoch Marker (unlike the example in {{fig-ex-2}}.
...
The entire Epoch Tick List MAY be conveyed as a Bell-signed Epoch Marker.
...
A Bell-signed Epoch Marker is an Epoch Marker conveyed in a signed data structure whose signature was generated by an Epoch Bell and cryptographically covers that Epoch Marker.
When a recipient needs to establish that an Epoch Marker was issued by an authorized Epoch Bell, the Epoch Marker MUST be Bell-signed Epoch Marker and its Bell-signed signature MUST be validated.

An Epoch ID value is often intended to be extracted from a Bell-signed Epoch Marker and included in another protected message, or conveyed via a secure channel.
Protection of that other message does not by itself make the extracted value a Bell-signed Epoch Marker, nor does it make its signer an Epoch Bell.
...
An X.509 certificate for an Epoch Bell signing key MUST assert this specific purpose using the `id-kp-epochBell` extended key purpose identifier, defined below, in the certificate's Extended Key Usage (EKU) extension {{Section 4.2.1.12 of -PKIX}}.

Certificates conforming to this specification:

* MUST include the EKU extension, marked critical, containing `id-kp-epochBell`;
...
Relying parties that require X.509-certified Epoch Bells MUST reject Bell-signed Epoch Markers whose signing certificate does not satisfy these constraints.
...
-- Epoch Bell extended key purpose

id-kp-epochBell OBJECT IDENTIFIER ::= { id-kp TBD12 }

END
~~~
{: #fig-epoch-bell-eku title="Epoch Bell Extended Key Usage ASN.1 Module"}
...
Freshness decisions derived from Bell-signed Epoch Markers depend on the Epoch Bell’s key(s) and correct behavior.
If the Epoch Bell key is compromised, or the Bell is malicious/misconfigured, an attacker can emit valid-looking “fresh” Bell-signed Epoch Markers.
...
Because Bell-signed Epoch Markers are only as trustworthy as the process that authorizes an entity to act as an Epoch Bell, relying parties need means to verify that a signing key was actually authorized for that purpose, and that the key is not shared with, or reused for, unrelated duties.
...
A network adversary can replay valid Bell-signed Epoch Markers or delay distribution, and differential latency can lead to different parties having different views of the “current” epoch.
...
* *Nonce-bound Bell interaction*: When a Verifier uses a nonce challenge to trigger Evidence creation, the Attester can forward that nonce to the Epoch Bell to request a Bell-signed Epoch Marker with the nonce echoed inside.
...
## New Media Type `application/epoch-marker+cbor`
...
IANA is requested to assign a new value for the Extended Key Usage defined in {{sec-epoch-bell-cert}} in the "SMI Security for PKIX Extended Key Purpose" registry of the "SMI Numbers" {{!IANA.smi-numbers}} registry group, as follows:

| Decimal | Description | References |
| TBD12 | id-kp-epochBell | {{sec-epoch-bell-cert}} of {{&SELF}} |
{: align="left" title="New PKIX Extended Key Purpose"}

IANA is requested to assign a new value for the ASN.1 Module defined in {{sec-epoch-bell-cert}} in the "SMI Security for PKIX Module Identifier" registry of the "SMI Numbers" {{!IANA.smi-numbers}} registry group:

| Decimal | Description | References |
| TBD11 | id-mod-epoch-bell | {{sec-epoch-bell-cert}} of {{&SELF}} |
